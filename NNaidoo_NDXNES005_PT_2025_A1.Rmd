...
# Merge into single timeSeries object
```{r}
# -----------------------------
...
# Tangency Portfolio (no short-selling)
```{r}
# -----------------------------
# 5) Tangency Portfolio (no short-selling)
# -----------------------------
compute_tangency_noshort <- function(mu, Sigma, rf=0, targets=seq(0.001,0.05,length.out=200)){
  n <- length(mu)
...
compute_tangency_noshort <- function(mu, Sigma, rf=0, targets = seq(0.001, 0.05, length.out=200)){
  n <- length(mu)
  Dmat <- 2*(Sigma + diag(1e-8, n))  # make PD
  best <- list(sharpe=-Inf)
  for(tgt in targets){
    gamma <- as.numeric(mu - rf)
    Amat <- cbind(gamma, -gamma, rep(1,n), -rep(1,n), diag(n))
...
### Rolling IS OOS Experiment
```{r}
# -----------------------------
# 6) Rolling IS/OOS Experiment
# -----------------------------
train_months <- 60
test_months  <- 12
roll_step    <- 12
n_obs <- nrow(rets_opt)
start_idxs <- seq(1, n_obs - train_months - test_months + 1, by=roll_step)
results <- list()

for(i in seq_along(start_idxs)){
  s <- start_idxs[i]
  train_idx <- s:(s+train_months-1)
  test_idx  <- (s+train_months):(s+train_months+test_months-1)
  
  train_rets <- rets_opt[train_idx, , drop=FALSE]
  test_rets  <- rets_opt[test_idx, , drop=FALSE]
  
  # Skip if any NAs in train/test
  if(any(!is.finite(as.matrix(train_rets))) || any(!is.finite(as.matrix(test_rets)))) next
 ...
 Sigma_train <- cov(as.matrix(train_rets), use="complete.obs")
  ...
  tang <- compute_tangency_noshort(mu=mu_train, Sigma=Sigma_train, rf=rf_train, targets=targ_grid)
  if(is.null(tang$weights)) next
  
  w_hat <- tang$weights
  port_train <- as.numeric(as.matrix(train_rets) %*% w_hat)
  port_test  <- as.numeric(as.matrix(test_rets) %*% w_hat)
  
  results[[length(results)+1]] <- list(
    train_period = paste(index(train_rets)[1], index(train_rets)[nrow(train_rets)], sep=" / "),
    test_period  = paste(index(test_rets)[1], index(test_rets)[nrow(test_rets)], sep=" / "),
...
### Buy-and-Hold portfolio weights table
```{r}
...
### BH Portfolio Simualtion
```{r}
# ==============================
# 8) Extra MATLAB-style Buy-and-Hold (BH) backtest
# ==============================
# This section replicates the MATLAB logic: compute portfolio returns, update weights due to price changes, 
# renormalise, and accumulate portfolio value over the test period
...
for(idx in seq_along(results)){
  ...
  # Get OOS returns for this window
  test_rets <- rets_opt[index(rets_opt) %in% index(rets_opt)[(train_months+1):(train_months+test_months)], , drop=FALSE]
  test_rets <- rets_opt[index(rets_opt) %in% index(rets_opt)[((idx-1)*roll_step+train_months+1):((idx-1)*roll_step+train_months+test_months)], , drop=FALSE]
  
  # Initial weights from tangency portfolio
  w0 <- results[[idx]]$weights
  
  n_assets <- ncol(test_rets)
  n_obs <- nrow(test_rets)
  
  # Initialise matrices
  Wts <- matrix(0, nrow=n_obs, ncol=n_assets)
  Ret <- matrix(0, nrow=n_obs, ncol=n_assets)
  portRet <- numeric(n_obs)
  portPrc <- numeric(n_obs)
  portPrc[1] <- 1       # initial wealth
  
  Wts[1,] <- w0          # set initial weights
  
  for(t in 1:(n_obs-1)){
    # 1. Compute returns (per asset)
    Ret[t+1,] <- as.numeric(test_rets[t+1,] / test_rets[t,] - 1)
    
    # 2. Portfolio return at t+1
    portRet[t+1] <- sum(Wts[t,] * Ret[t+1,])
    
    # 3. Update weights due to price change
    Wts[t+1,] <- Wts[t,] * (1 + Ret[t+1,])
    
    # 4. Renormalise (ensure sum = 1)
    Wts[t+1,] <- Wts[t+1,] / sum(Wts[t+1,])
    
    # 5. Update cumulative wealth
    portPrc[t+1] <- portPrc[t] * (1 + portRet[t+1])
  }
  ...
  # Plot portfolio cumulative wealth
  plot_df <- data.frame(
    Date=index(test_rets),
    Wealth=portPrc
  )
  
  ggplot(plot_df, aes(x=Date, y=Wealth)) +
    geom_line(color="blue") +
    labs(title=paste0("Buy-and-Hold Tangency Portfolio - Window ", idx),
         x="Date", y="Cumulative Wealth") +
    theme_minimal() -> p
  print(p)
}
...
### Plot weights evolution over OOS
```{r}
# ==============================
# 9) Plot weights evolution over OOS
# ==============================

for(idx in seq_along(BH_results)){
  Wts <- BH_results[[idx]]$weights
  dfW <- data.frame(Date=BH_results[[idx]]$dates, Wts)
  colnames(dfW)[-1] <- BH_results[[idx]]$asset_names
  
  df_long <- tidyr::pivot_longer(dfW, cols=-Date, names_to="Asset", values_to="Weight")
  
  ggplot(df_long, aes(x=Date, y=Weight, color=Asset)) +
    geom_line() +
    labs(title=paste0("Buy-and-Hold Weights Evolution - Window ", idx),
         y="Portfolio Weight", x="Date") +
    theme_minimal() -> p
  print(p)
}
...
### Sanity check: final cumulative return
```{r}
# ==============================
...
# ==============================
# 11) Extra MATLAB-style: Combined Buy-and-Hold Wealth Plot
# ==============================
...
# Combine all windows into a single data frame
combined_df <- data.frame()

for(idx in seq_along(BH_results)){
  df_tmp <- data.frame(
    Date = BH_results[[idx]]$dates,
    Wealth = BH_results[[idx]]$portPrc,
    Window = paste0("Window_", idx)
  )
  combined_df <- rbind(combined_df, df_tmp)
}

# Pivot for ggplot if needed
combined_df$Window <- as.factor(combined_df$Window)

# Plot all BH wealth curves together
ggplot(combined_df, aes(x=Date, y=Wealth, color=Window)) +
  geom_line(size=1) +
  labs(
    title = "Combined Buy-and-Hold Portfolio Wealth Across Windows",
    x = "Date",
    y = "Cumulative Wealth",
  ...
  scale_color_brewer(palette="Set1") -> p_combined
